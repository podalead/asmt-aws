...
case ${COMMAND} in
    ...
    'test')
        ...
        terraform init \
            -reconfigure \
            -backend-config="path=${ROOT_PATH}/state/${PROFILE}/${PREFIX}/fargate_backend.tfstate"
    ...
    'deploy' | 'cleanup')
        ...
        terraform init \
            -reconfigure \
            -backend-config="path=${ROOT_PATH}/state/${PREFIX}/${PREFIX}/fargate_backend.tfstate"
        ...
        if [[ "deploy" == "${COMMAND}"  ]]; then
                    terraform plan \
                        ...
                        -input=false \
                        -refresh=true \
                        -var-file="${ROOT_PATH}/profile/${PROFILE}/tf_vars.tfvars" \
                        -var-file="${ROOT_PATH}/profile/${PROFILE}/aws_cred.tfvars" \
                        -var="prefix=${PREFIX}" \
                        -var="profile=${PROFILE}" \
                        -var="pipe_root=${ROOT_PATH}"
